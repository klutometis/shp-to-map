         ...
         (append
          (hash-table-fold
           ...
           (lambda (name name->geometries names->geometries)
             (append (hash-table->alist name->geometries)
                     names->geometries))
           '())
          types->name))
       '()))
    ...
    (let ((country-code->type (make-hash-table)))
      (call-with-postgresql-connection
       ...
       (lambda (connection)
         ...
         (row-for-each*
          (lambda (country-code type name geometry)
            (debug country-code type name geometry)
            (hash-table-update!/default
             ...
             (lambda (type->name)
               ...
               type->name)
             (make-hash-table)))
          (query connection "SELECT country_code, type, name, substring(asBinary(geometry) from 1 for 10) FROM places_having_names(100, 2000000);"))))
