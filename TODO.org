         ...
         (row-for-each*
          (lambda (country-code class name geometry)
            ...
            (let* ((name (vector->list name))
              ...
              (debug name geometry)))
          (query connection
                 "select country_code, class, get_name_by_language(name, '{\"name:en\"}'), substring(asBinary(geometry) from 1 for 10) from placex where class = 'place' and name is not null and admin_level < 100 limit 1000;")))))
