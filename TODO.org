...
* TODO Parse city, state, country files, check against Factual data.
  ...
  #+BEGIN_SRC java :tangle test-against-countries.bsh :shebang #!/usr/bin/env chicken-scheme
    addClassPath("lib/guava-10.0.1.jar");
    addClassPath("lib/jts-1.12.jar");
    ...
    Files.readLines(new File("us_address_sample_2.tab"),
                    Charsets.UTF_8,
                    new LineProcessor() {
            geometryFactory = new GeometryFactory();
            hits = misses = nulls = 0;
            ...
            public getResult() {
                print("hits: " + hits +
                      " misses: " + misses +
                      " nulls: " + nulls +
                      " total: " + (hits + misses + nulls) +
                      " percent hit: " + hits / (hits + misses + 0.0));
                return hits / (hits + misses + 0.0);
            }
    
            public processLine(line) {
                try {
                    data = line.split("\t");
                    city = normalizeKey(data[2]);
                    latitude = Double.valueOf(data[5]);
                    longitude = Double.valueOf(data[6]);
                    geometries = cities.get(city);
                    if (geometries == null) {
                        nulls++;
                    } else {
                        point = new Point(new CoordinateArraySequence(new Coordinate[] {
                                    new Coordinate(longitude, latitude)
                                }),
                            geometryFactory);
