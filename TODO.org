              ...
              (if (pair? names)
                  (hash-table-update!/default
                   ...
                   (lambda (type->name)
                     ...
                     type->name)
                   (make-hash-table)))))
          (query connection "SELECT upper(country_code), type, name, substring(asBinary(geometry) from 1 for 10) FROM placex WHERE country_code IS NOT NULL AND name IS NOT NULL AND type IS NOT NULL AND class = 'place' LIMIT 1000;"))))
