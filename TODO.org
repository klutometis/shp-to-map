    ...
    (let ((country-code->class (make-hash-table)))
      (call-with-postgresql-connection
       ...
       (lambda (connection)
         ...
         (row-for-each*
          (lambda (country-code class name geometry geometry-raw)
            (debug country-code class name geometry geometry-raw)
            (with-output-to-file
                "geometry-raw.txt"
              (lambda () (display geometry-raw))))
          (query connection "SELECT country_code, class, name, encode(asBinary(geometry), 'hex'), geometry FROM places_having_names(100, 1);")))))
