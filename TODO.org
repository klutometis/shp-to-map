          ...
          (lambda (country-code type names geometry)
            ...
            (let* ((names-sans-ref
                    (remove (lambda (key-value)
                            ...
                            (vector->list names)))
                   (names (begin (debug names-sans-ref
                                        (map (compose string-trim-both cdr) names-sans-ref)
                                        (map string->list (map cdr names-sans-ref)))
                                 (map (compose string-trim-both cdr) names-sans-ref)))
                   (non-empty-names (remove string-null? names)))
              (if (pair? names)
                  (hash-table-update!/default
                   ...
                   (lambda (type->name)
                     ...
                     type->name)
                   (make-hash-table)))))
          (query connection "SELECT upper(country_code), type, name, substring(asBinary(geometry) from 1 for 10) FROM placex WHERE name IS NOT NULL AND type IS NOT NULL AND class = 'place' LIMIT 1000;"))))
      (debug (country-code->type->alist country-code->type))
      )
  #+END_SRC
