       ...
       (lambda (connection)
         ...
         (row-for-each*
          (lambda (country-code type names geometry)
            (let ((names (map cdr (vector->list names))))
              (hash-table-update!/default
               ...
               (lambda (type->name)
                 ...
                 type->name)
               (make-hash-table))))
          (query connection "SELECT upper(country_code), type, name, substring(asBinary(geometry) from 1 for 10) FROM placex WHERE name IS NOT NULL and class = 'place';"))))
