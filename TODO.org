...
* TODO Map file for each country
  ...
  #+BEGIN_SRC scheme :tangle places-by-country.scm :shebang #!/usr/bin/env chicken-scheme
    (use debug postgresql srfi-69 matchable)
    ...
    (let ((country-code->class (make-hash-table)))
      (call-with-postgresql-connection
       ...
       (lambda (connection)
         (row-for-each*
          (lambda (country-code class name get-name geometry)
            (debug name get-name)
            #;
            (let* ((name (vector->list name))
                   (name (alist-ref "name" name)))
              (debug name geometry)))
          (query connection
                 "select country_code, class, name, get_name_by_language(name, '{\"place_name:en\", \"place_name\", \"name:en\", \"name\", \"int_name\"}'), substring(asBinary(geometry) from 1 for 10) from placex where class = 'place' and get_name_by_language(name, '{\"place_name:en\", \"place_name\", \"name:en\", \"name\", \"int_name\"}') is not null and admin_level < 100 limit 1000;")))))
  ...
  #+BEGIN_SRC sql
    ...
    SELECT country_code, class, name, substring(asBinary(geometry) from 1 for 10)
           FROM places_having_names(100, 100);
  #+END_SRC
